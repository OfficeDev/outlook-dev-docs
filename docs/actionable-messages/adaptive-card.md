...
> [!IMPORTANT]
> The Adaptive Card format is **required** to support Outlook on iOS and Android. The MessageCard format is still supported but is now de-emphasized.
>
...
| `title` | String | No | The title of the action as it will appear on screen on a button control, for instance. |
| `addInId` | String | Yes | Specifies the add-in ID of the required add-in. The location of the add-in ID in the manifest varies depending on the type of manifest the add-in uses.<ul><li>**Add-in only manifest**: The [\<Id> element](/office/dev/add-ins/reference/manifest/id).</li><li>**Unified manifest for Microsoft 365**: The ["id" property](/microsoft-365/extensibility/schema/root#id) of the root object.</li></ul> |
| `desktopCommandId` | String | Yes | Specifies the ID of the add-in command button that opens the required task pane. The location of the command button ID in the manifest varies depending on the type of manifest the add-in uses.<ul><li>**Add-in only manifest**: The `id` attribute of the [\<Control> element](/office/dev/add-ins/reference/manifest/control) element that defines the button. The specified `<Control>` element MUST be defined inside a [MessageReadCommandSurface extension point](/office/dev/add-ins/reference/manifest/extensionpoint), be of type `Button`, and the control's `<Action>` element must be of type `ShowTaskPane`.</li><li>**Unified manifest for Microsoft 365**: The `"id"` property of the ["extensions.ribbons.tabs.groups.controls" object](/microsoft-365/extensibility/schema/extension-common-custom-group-controls-item#id) that defines the button. The specified control must be defined in a ["mailRead" context](/microsoft-365/extensibility/schema/extension-ribbons-array#contexts), be of type `button`, and the ["extensions.runtimes.actions.type"](/microsoft-365/extensibility/schema/extension-runtimes-actions-item#type) must be set to `openPage`. </li></ul> |

...
Let's start by creating an actionable message card. We'll start with something simple, just a basic card with an `Action.Http` action and an `Action.OpenUrl` action. We'll use the [Card Playground](https://amdesigner.azurewebsites.net/) to design the card.

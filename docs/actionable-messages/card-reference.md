...
| `correlationId` | UUID | The `correlationId` property simplifies the process to locate logs for troubleshooting issues. We recommend to set and log unique UUID while sending an email or a connector message.<br><br>In addition, we also send `Card-Correlation-Id` and `Action-Request-Id` headers in action execution request to your endpoint. `Card-Correlation-Id` contains the same value as of `correlationId` property in message card. `Action-Request-Id` is a unique UUID generated by us to help locate specific action performed by a user. |

...
| `expectedActors` | Array of `String` | Optional. This contains list of expected email addresses of the recipient for the action endpoint.<br><br>A user can have multiple email addresses and the action endpoint might not be expecting the particular email address presented in `sub` claim of the bearer token. For example, `johne.doe@contoso.com` or `jdoe@contoso.com` can represent the same email address, but developer always want to receive "jdoe@contoso.com" as `sub` claim of the bearer token then this field can be set as `[ "jdoe@contoso.com"]`.|

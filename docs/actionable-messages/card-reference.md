...
| `correlationId` | UUID | Set unique UUID for troubleshooting purposes.<br><br>**Recommended** to log before sending email or connector message.<br>For actions troubleshooting, **recommended** to log request header 'Card-Correlation-Id' and 'Action-Request-Id' in action execution request to your service endpoint. |

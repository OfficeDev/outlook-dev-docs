...
| `correlationId` | UUID | Set correlationId property for troubleshooting purposes.<br><br>**Recommended** to log before sending email or connector message.<br>To troubleshoot actions, **recommended** to log request header 'Card-Correlation-Id' and 'Action-Request-Id' in action execution request to your service endpoint.|

...
| `summary` | String | Required if the card does not contain a `text` property, otherwise optional. The `summary` property is typically displayed in the list view in Outlook, as a way to quickly determine what the card is all about.<br><br>**Do** always include a summary.<br>**Don't** include details in the summary. For example, for a Twitter post, a summary might simply read "New tweet from @someuser" without mentioning the content of the tweet itself. |

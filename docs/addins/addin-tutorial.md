...
This tutorial will use the [Microsoft Office Project Generator Beta](https://github.com/officedev/generator-office/tree/beta) to generate an add-in project.
...
- [Microsoft Office Project Generator Beta](https://github.com/officedev/generator-office/beta)
...
- **Would you like to create a new project?:** `Yes, I want a new project`
...
The generator will create the project and install supporting Node components. When it is done, it will prompt asking if you want to open resource.html. It isn't necessary to open it for this tutorial, but feel free to open it if you're curious!

> [!NOTE]
> In the current beta, we need to fix the manifest before it will load:
> 
> 1. Find the `<bt:Url>` element with `id="functionFile"` and remove the `~` from the beginning of the URL.
...
1. In your command prompt/shell, make sure you are in the root directory of your project, and enter `npm start`. This will start a web server at `https://localhost:3000` and open your default browser to that address.
    - If your browser indicates that the site's certificate is not trusted, you will need to add the certificate as a trusted certificate. Outlook will not load add-ins if the site is not trusted. See [Adding Self-Signed Certificates as Trusted Root Certificate](https://github.com/OfficeDev/generator-office/blob/beta/src/docs/ssl.md) for details.
...
The manifest for an add-in controls how it appears in Outlook. It defines the way the add-in appears in the add-in list, the buttons that appear on the ribbon, and it sets the URLs for the HTML and JavaScript files used by the addin.
...
1. Open the `git-the-gist-manifest.xml` file. Locate the `ProviderName` element in the XML and replace the default value with your company name.
    ...
    ```
1. Update the `Description` with a description of the addin.
    ```xml
...
Let's start by creating our function file. Any functions that are invoked by a UI-less button must be defined in the function file specified by the `FunctoinFile` element in the manifest for the corresponding form factor. Our manifest has this defined as `https://localhost:3000/function-file/function-file.html`. Create a folder named `function-file` in the root of the project, then create `function-file.html` insdie that folder. Add the following code.
...
<head>
    ...
    <script type="text/javascript" src="../helpers/gist-api.js"></script>
    <script src="function-file.js" type="text/javascript"></script>
</head>
<body>
...
This makes use of the [`RoamingSettings` object](https://dev.office.com/reference/add-ins/outlook/RoamingSettings) to get or set the configuration values.

Now create a file named `function-file.js` in the `function-file` folder, and add the following code:
...
Click the **Settings** button and enter your GitHub username. Press **Tab** to invoke the `change` event, which should load your list of gists. Select a gist to be the default, and click **Done**.

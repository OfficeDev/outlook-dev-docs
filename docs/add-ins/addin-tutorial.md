...
After you've reinstalled the add-in, you can verify that it installed successfully by checking for the options **Insert gist** and **Insert default gist** in the message compose window. Note that nothing will happen if you select either of these items, because you haven't yet specified the code that runs when these items are selected.

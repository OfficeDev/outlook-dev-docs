...
The payment request webhook is called when the user starts the payment experience by choosing the "Pay" button on your message card. Additionally, if you request shipping information, the webhook will also be called multiple times during the payment experience, allowing you to update the payment information presented to the user based on selections they make.

The format of the payload sent to your webhook and the payload you must send back in the response follow the same schema, specified in the following section. However, certain fields may not be present depending on the context. Details on which fields will be present can be found in the relevant sections.
...
| Field | Type| Description |
|-------|-----|-------------|
| `merchantId` | String | The merchant ID generated by the Payments in Outlook Partner Dashboard. |
...
| `entity` | Schema.org [Invoice](http://schema.org/Invoice) | Represents the invoice. |
| `mode` | String | If present and set to `TEST`, enables test mode. In this mode, the recipient has access to pre-configured test cards in the payment experience. Payment tokens in test mode are test tokens as defined in [Stripe Test Cards and numbers](https://stripe.com/docs/testing#cards). |

###### Example

```json
{
  "merchantId": "cf35d68f-bc07-49e8-b63e-116de54745f8",
  "supportedNetworks": [
    "visa",
    "mastercard"
  ],
...
After loading the invoice, your webhook should perform the following key steps to modify the request payload to generate a response payload:

- Update the `data` property to add information about accepted payment types and networks and a Schema.org Invoice entity.
- Add the `options` property to indicate what information you need about the payer and if you want the Payments service to ask for a shipping address and option.
- Add the `details` property to provide display details about the invoice.

...
1. Make **sure** you are in the **master** branch. All pull requests **MUST** go to the **master** branch.
...
1. To submit a pull request using Git, follow the steps in [Use GitHub, Git, and this repository](#use-github-git-and-this-repository). Be **sure** to send pull requests to the **master** branch of this repository. All pull requests **MUST** go to the **master** branch.
...
1.	Install Git on your computer. Follow the steps in the [Setting up Git Tutorial](https://help.github.com/articles/set-up-git/).
...
1. Submit a pull request to the **master** branch of the main repository.
1. Once the pull request is merged, delete the branch.
...
1.	Open Git Bash.
1.	At the Git Bash command prompt, type `git pull upstream master:<new branch name>`. This creates a new branch locally that is copied from the latest OfficeDev master branch.
...
1.	Verify the Base branch is `OfficeDev/<repo name>@master` and the Head branch is `<your username>/<repo name>@<branch name>`.
...
For example, if your local branch X was successfully merged into the OfficeDev/outlook-dev-docs master branch and you want to make additional updates to the content that was merged. Create a new local branch, X2, from the OfficeDev/outlook-dev-docs master branch. To do this, open GitBash and execute the following commands:
...
...and verifying the content. (The `checkout` command updates the files in `C:\Users\<yourusername>\outlook-dev-docs` to the current state of the X2 branch.) Once you check out the new branch, you can make updates to the content and commit them as usual. However, to avoid working in the merged branch (X) by mistake, it's best to delete it (see the following **Delete a branch** section).
